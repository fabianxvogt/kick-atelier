...
export function levelMatch(samples, ceiling = 0.8912) { let peak = 0; for (const value of samples) peak = Math.max(peak, Math.abs(value)); if (!Number.isFinite(peak) || peak === 0) return samples; const factor = Math.min(1, ceiling / peak); if (factor === 1) return samples; return Float32Array.from(samples, (value) => value * factor); }
